...
@DriftDatabase(tables: [Tasks, Tags, Quotes], daos: [TaskDao, TagDao, QuoteDao])
...
@DriftAccessor(
  ...
  queries: {'completedTasksGenerated': 'SELECT * FROM tasks WHERE completed = 1 ORDER BY due_date DESC, name;'},
)
class TaskDao extends DatabaseAccessor<AppDatabase> with _$TaskDaoMixin {
  ...
  Stream<List<Task>> watchCompletedTasks() {
    return (select(tasks)
          ..orderBy([
            (tbl) => OrderingTerm(expression: tbl.dueDate, mode: OrderingMode.desc),
